  ...
  public static RestDocumentationResultHandler searchKeywordRestDocument() {
    return MockMvcRestDocumentation.document("blog-search",
        ...
        queryParameters(
            ...
            parameterWithName("size").optional().description("한 페이지에 보여질 문서 수, 1~50 사이의 값, 기본 값 10")
        ),
        responseFields(
            fieldWithPath("documents[].blogName").description("블로그 명"),
            fieldWithPath("documents[].contents").description("블로그 컨텐츠"),
            fieldWithPath("documents[].localDateTime").description("게시글 작성 시간"),
            ...
            fieldWithPath("pageInfo.totalCount").description("총 조회 게시물 수"),
            fieldWithPath("pageInfo.pageableCount").description("노출 가능한 게시물 수"),
            fieldWithPath("pageInfo.lastPage").description("마지막 페이지 여부")
        )
    );
  }
}
